                                    ...
                                    <div class="card-body">
                                        <table class="table table-bordered table-striped table-hover" id="loan_table">
                                            ...
                                            <tbody>
                                                <?php
                                                $count = 1;
                                                    $sql = mysqli_query($conn, "SELECT *, cd.id as cid FROM client_data cd, emp_data ed, loan_data ld, nominee_data nd WHERE ld.status = 'Active' AND ld.client_id = cd.id AND ld.emp_id = ed.id AND cd.nominee_id = nd.id");

                                                    while($loan = mysqli_fetch_assoc($sql)){
                                                ?>
                                                <tr onclick='window.location.href="Client-Profile/<?php echo $loan["cid"];?>/"' style="cursor:pointer;">
                                                    <td><?php echo $count++;?></td>
                                                    <td><?php echo $loan['name'];?></td>
                                                    <td><?php echo date('d-m-Y', strtotime($loan['loan_date']));?></td>
                                                    <td><?php echo $loan['loan_amount'];?></td>
                                                    <td><?php echo $loan['nominee_name'];?></td>
