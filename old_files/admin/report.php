                            ...
                            <div id="kt_app_content_container" class="app-container  container-xxl ">
                                <div class="card mb-4">
                                    <div class="card-body row">
                                        ...
                                        <div class="col-lg-4 col-12 pt-3">
                                            <div class="form-group">
                                                <label for="">Collecting Person</label>
                                                <select name="" id="emp" class="form-select">
                                                    ...
                                                    while($ed = mysqli_fetch_assoc($sql)){
                                                        ?>
                                                        <option value="<?php echo $ed['id'];?>"><?php echo $ed['emp_name'];?></option>
                                                        <?php };?>
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <label for="">Client</label>
                                                <select name="" id="client" class="form-select">
                                                    ...
                                                    <?php
                                                        ...
                                                        $sql = mysqli_query($conn, "SELECT * FROM client_data ed WHERE ed.status = 'Active'");
                                                        while($ed = mysqli_fetch_assoc($sql)){
                                                        ?>
...
<script>
    $(document).ready(function(){
        ...
        $("#get_report").click(function(){
            ...
            $.ajax({
                url:'<?php echo $path;?>ajax/admin/report.php',
                processData: false,
                contentType: false,
                data: fd,
                type: 'post',
                success: function(response){
                   ...
                   if(res.status = 'Success')
                   {
                    ...
                    rec.map((array, index) => 
                        {
                           $("#table_result").append(`
                                <tr style="cursor:pointer;" onclick="window.location='Client-Profile/${array.loan_client_id}/'">
                                    <td>`+ sno++ +`</td>
                                    <td>${array.name}</td>
                                    <td>${new Date(array.payment_date).toLocaleDateString('en-GB')}</td>
